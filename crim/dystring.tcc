...
public:
     ...
     *          `src.m_pbuffer` is not freed, allowing us to keep it around!
     */
     ...
     *          `this->m_pbuffer` & `src.m_pbuffer` are distinct blocks of memory.
     * 
     * @details `base::m_nlength` doesn't include the nul char in its count, we
     ...
     * 
     *          Does a shallow copy of `src`'s data. So `this->m_pbuffer` points
     *          to the same data that `src.m_pbuffer` does.
     * 
     ...
     * 
     * @note    `base::m_nlength` doesn't include the nul char in its count, we
     *          have to explicitly append it ourselves else we might end up with
     *          a likely buffer overrun!
     */
     ...
     *          to a nul char. That function takes care of decrementing index,
     *          and destroying the object at that index.
     */
    CharT pop_back() {
    ...
    dystring &append(const CharT *msg) {
        // Don't add 1 to avoid both writing nul char and incrementing index.
        for (size_t i = 0, len = strlen(msg); i < len; i++) {
            base::push_back(msg[i]); // faster than append() since no nul char
        }
        return append('\0');
    }
    ...
    dystring &append(CharT c) {
        ...
        base::m_nlength--; // Decrement so we ignore nul chars in our count.
        base::m_iterator.m_end--; // ^ditto.
        return *this;
    }
};
